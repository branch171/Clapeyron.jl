...
model2 = SAFTVRMieKiselev(["carbon dioxide"];idealmodel=AlyLeeIdeal)

function set_parameters_Kiselev!(model, p)
    segment,sigma,epsilon,lambda_r,lambda_a = p[1],p[2],p[3],p[4],p[5]
    model.basemodel.params.segment[1] = segment
    model.basemodel.params.sigma[1] = sigma*1.0e-10
    model.basemodel.params.epsilon[1] = epsilon
    model.basemodel.params.lambda_r[1] = lambda_r
    model.basemodel.params.lambda_a[1] = lambda_a
    Clapeyron.recombine!(model.basemodel)
...
#p = zeros(5)
#p[1],p[2],p[3],p[4],p[5] = 2.1921295915220615, 2.7603463754764865, 173.54383419960809, 18.947806879264714, 5.258420355533501
#set_parameters_Kiselev!(model2, p)
