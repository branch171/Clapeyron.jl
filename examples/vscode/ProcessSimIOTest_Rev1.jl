...
function Get_TotalEnthalpy_FlashTp(res)
    np = round(Int64, res[1])
    ires = 1
    beta = Vector{Float64}(undef,0)
    for ip = 1:np
...
@register_symbolic Get_TotalEnthalpy_FlashTp(res::AbstractVector)
...
function Get_TotalEntropy_FlashTp(res)
    np = round(Int64, res[1])
    ires = 1
    beta = Vector{Float64}(undef,0)
    for ip = 1:np
...
@register_symbolic Get_TotalEntropy_FlashTp(res::AbstractVector)
...
@component function MassSource(;name, ps, Ts, mdots, zs)
    ...
    #    s ~ Get_Entropy_FlashTp(flash,1)*Get_Beta_FlashTp(flash,1) + Get_Entropy_FlashTp(flash,2)*Get_Beta_FlashTp(flash,2)
        h ~ Get_TotalEnthalpy_FlashTp(flash)
        s ~ Get_TotalEntropy_FlashTp(flash)
...
@component function Separator(;name, pg, Tg, zg)
    ...
    #    s_in ~ Get_Entropy_FlashTp(flash,1)*Get_Beta_FlashTp(flash,1) + Get_Entropy_FlashTp(flash,2)*Get_Beta_FlashTp(flash,2)
        h_in ~ Get_TotalEnthalpy_FlashTp(flash)
        s_in ~ Get_TotalEntropy_FlashTp(flash)
