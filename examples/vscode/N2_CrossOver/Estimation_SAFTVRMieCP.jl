...
model  = SAFTVRMieCP(["nitrogen"];idealmodel=AlyLeeIdeal);
Clapeyron.recombine!(model)

segment0,sigma0,epsilon0,lambda_r0,lambda_a0 = 1.5461614648849995, 3.0491643271702293, 85.36164814660276, 6.386943372742874, 5.885515423346442
...
function saturation_p_rv_rl(model::EoSModel,T)
    ...
    Tsat = 0.0
    psat = 0.0
    vl = 3.2297209260022036e-5
    vv = 0.04192756714577973
    v0 = [vl,vv]
    for i in 1:N
...
estimator,objective,initial,upper,lower = Estimation(model,toestimate,[("/home/cbranch/julia/dev/Clapeyron.jl/examples/data/N2/saturation_p_rv_rl.csv")]);
