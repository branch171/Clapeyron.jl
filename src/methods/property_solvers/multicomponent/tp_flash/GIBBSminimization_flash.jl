...
function ConstraintsGIBBS_Gibbs(x,np,n₀,lb,ub,lbrho,ubrho,s)
    n = size(x)[1]
...
function ProjectionGIBBS_base(x,lb,ub)
    n = size(x)[1]
    p = Vector{Base.promote_eltype(x)}(undef,n)
    for i = 1:n
		p[i] = x[i]
...
function ProjectionGIBBS_Gibbs(x,np,n₀,lb,ub,lbrho,ubrho)
	n = size(x)[1]
...
#	println("ProjectionGIBBS_Gibbs x  = $(x)")
...
#	println("ProjectionGIBBS_Gibbs outside  = $(outside)")
#	println("ProjectionGIBBS_Gibbs beta  = $(beta)")
#	println("ProjectionGIBBS_Gibbs xp  = $(xp)")
#	println("ProjectionGIBBS_Gibbs rhop  = $(rhop)")
...
#		println("ProjectionGIBBS_Gibbs p  = $(p)")
...
function GIBBS_Gibbs_func(model,p,T,n₀,v₀,np,x)
	nc = length(n₀)
	beta = Vector{eltype(x)}(undef,0)
	xp = Vector{Vector{eltype(x)}}(undef,0)
...
function GIBBS_initial_compositions(model,p,T,z,add_pure_guess,add_anti_pure_guess,add_pure_component,add_random_guess,add_all_guess)
	n = length(z)
    ...
    sumxvap = sum(xvap)
    xvap ./= sumxvap
    xvap = ProjectionGIBBS_base(xvap,lb,ub)
    sumxvap = sum(xvap)
    xvap ./= sumxvap
    push!(xp,xvap)
    xliq = fill(1.,n)
    ...
    sumxliq = sum(xliq)
    xliq ./= sumxliq
    xliq = ProjectionGIBBS_base(xliq,lb,ub)
    sumxliq = sum(xliq)
    xliq ./= sumxliq
    push!(xp,xliq)
    ...
    if add_anti_pure_guess || add_all_guess
    	...
    	for i in 1:n
    		if add_pure_component[i] == true
        		...
        		sumxi = sum(xi)
        		xi ./= sumxi
        		xi = ProjectionGIBBS_base(xi,lb,ub)
    			sumxi = sum(xi)
    			xi ./= sumxi
        		push!(xp,xi)
        		# anti pure Wilson
...
function GIBBS_Pereira_compositions(model,p,T,z)
	...
	for id in eachindex(d)
		for i = 1:n-1
			...
			x̂ = ProjectionGIBBS_base(x̂,lb,ub)
			sumx̂ = sum(x̂)
			x̂ ./= sumx̂
			push!(xp,x̂)
			x̄ = ProjectionGIBBS_base(x̄,lb,ub)
			sumx̄ = sum(x̄)
			x̄ ./= sumx̄
			push!(xp,x̄)
		end
...
#	println("GIBBS Step 1 - vref = $(vref) vref_crit = $(vref_crit)")
    ...
    xi = GIBBS_initial_compositions(model,p,T,z₀,add_pure_guess,add_anti_pure_guess,add_pure_component,add_random_guess,add_all_guess)
    ...
    if stable
		...
		xm = GIBBS_Pereira_compositions(model,p,T,z₀)
		...
		#	λᵁ = fill( λmax,nc-1)
			...
			if use_global_solution
				...
				for iter_rand = 1:iter_rand_max
    				...
    				sumxr = sum(xr)
    				xr ./= sumxr
    				xr = ProjectionGIBBS_base(xr,lb,ub)
    				sumxr = sum(xr)
    				xr ./= sumxr
...
#    				end
    	...
    	# we made it here without a GIBBS solution just return single phase and warn 	
			...
			Gibbs(x) = GIBBS_Gibbs_func(model,p,T,z₀,vref,np,x)
			...
			push!(ub,15.0)
			projGibbs(x) = ProjectionGIBBS_base(x,lb,ub)
			...
			projGibbs(x) = ProjectionGIBBS_Gibbs(x,np,z₀,lb,ub,lbrho,ubrho)
			cnstGibbs(x,s) = ConstraintsGIBBS_Gibbs(x,np,z₀,lb,ub,lbrho,ubrho,s)
