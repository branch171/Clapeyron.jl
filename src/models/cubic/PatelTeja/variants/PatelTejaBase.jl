abstract type PatelTejaBaseModel <: ABCCubicModel end

const PatelTejaBaseParam = ABCBaseCubicParam

struct PatelTejaBase{T <: IdealModel,α,c,γ} <:PatelTejaBaseModel
    components::Array{String,1}
    alpha::α
    mixing::γ
    translation::c
    params::PatelTejaBaseParam
    idealmodel::T
    references::Array{String,1}
end

"""
    PatelTejaBase(components;
...
model = PatelTejaBase(["neon","hydrogen"];
...
export PatelTejaBase

function PatelTejaBase(components;
    idealmodel = BasicIdeal,
    ...
    params = getparams(formatted_components, ["properties/criticalbase.csv", "properties/molarmass.csv","SAFT/PCSAFT/PCSAFT_unlike.csv"]; userlocations = userlocations, verbose = verbose)
    model = CubicModel(PatelTejaBase,params,formatted_components;
...
default_references(::Type{PatelTejaBase}) = ["10.1016/0009-2509(82)80099-7"]

function ab_premixing(model::PatelTejaBaseModel,mixing::MixingRule,k,l)
    _Tc = model.params.Tc
    _pc = model.params.Pc
    _Zc = model.params.Zc0
    a = model.params.a
    b = model.params.b
    n = length(model)
#    _Zc = _pc .* _Vc ./ (R̄ .* _Tc)
...
function c_premixing(model::PatelTejaBaseModel)
    _Tc = model.params.Tc
    _pc = model.params.Pc
    _Zc = model.params.Zc0
    c = model.params.c
...
function cubic_Δ(model::PatelTejaBaseModel,z)
